...
export default function ProductCard({ product, index = 0 }) {
  ...
  const handleFavorite = (e) => {
    ...
    addToast(favorited ? 'Removed from favorites' : 'Added to favorites', 'info', favorited ? '♡' : '♥');
  };
    ...
    <>
      <article className={`p-card reveal reveal-delay-${(index % 5) + 1}`}>
        <div className="p-card__media">
          ...
          {outOfStock && <div className="p-card__sold-out">Sold Out</div>}
          {product.availability === 'low_stock' && (
            <div className="p-card__low-stock">Only {product.stock} left</div>
          )}
        </div>
