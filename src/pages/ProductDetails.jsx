      ...
      <div className="container">
        ...
        <div className="pd-grid">
          ...
          <div className="pd-gallery">
            <div className={`pd-gallery__main ${zoomed ? 'pd-gallery__main--zoomed' : ''}`} onClick={() => setZoomed(!zoomed)}>
              ...
              <span className="pd-gallery__zoom-hint">{zoomed ? 'Click to zoom out' : 'Click to zoom'}</span>
            </div>
          ...
          <div className="pd-info">
            ...
            <div className="pd-info__price">
              ...
              {discount > 0 && <span className="pd-info__discount">Save {discount}%</span>}
            </div>

            <div className="pd-info__availability">
              {outOfStock ? (
                <span className="pd-availability pd-availability--out">● Out of Stock</span>
              ) : product.availability === 'low_stock' ? (
                <span className="pd-availability pd-availability--low">● Only {product.stock} left in stock</span>
              ) : (
                <span className="pd-availability pd-availability--in">● In Stock ({product.stock} available)</span>
              )}
            </div>
            ...
            <div className="pd-actions">
              ...
              <button className="pd-btn pd-btn--fav" onClick={() => { toggleFavorite(product.id); addToast(favorited ? 'Removed from favorites' : 'Added to favorites', 'info', favorited ? '♡' : '♥'); }} aria-pressed={favorited}>
            ...
            <div className="pd-share">
              ...
              <div className="pd-share__btns">
                <button onClick={() => handleShare('whatsapp')} aria-label="Share on WhatsApp">WA</button>
                <button onClick={() => handleShare('facebook')} aria-label="Share on Facebook">FB</button>
                <button onClick={() => handleShare('twitter')} aria-label="Share on Twitter">X</button>
                <button onClick={() => handleShare('copy')} aria-label="Copy link">🔗</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pd-tabs">
          <div className="pd-tabs__nav">
            <button className={tab === 'description' ? 'pd-tab--active' : ''} onClick={() => setTab('description')}>Description</button>
            <button className={tab === 'care' ? 'pd-tab--active' : ''} onClick={() => setTab('care')}>Care Instructions</button>
            <button className={tab === 'shipping' ? 'pd-tab--active' : ''} onClick={() => setTab('shipping')}>Shipping</button>
          </div>
          <div className="pd-tabs__content">
            {tab === 'description' && product.description.split('\n\n').map((p, i) => <p key={i}>{p}</p>)}
            {tab === 'care' && (
              <ul>
                <li>Hand wash cold in gentle detergent, do not wring</li>
                <li>Lay flat to dry — do not hang or tumble dry</li>
                <li>Iron on low heat if needed, inside out</li>
                <li>Store folded, away from direct sunlight</li>
              </ul>
            )}
            {tab === 'shipping' && (
              <ul>
                <li>Processing time: 2–4 business days (handmade to order)</li>
                <li>Local delivery: 3–5 business days</li>
                <li>Orders are confirmed and arranged via WhatsApp</li>
        ...
        {related.length > 0 && (
          <div className="pd-related">
            <h2 className="t-display pd-related__title">You Might Also Love</h2>
